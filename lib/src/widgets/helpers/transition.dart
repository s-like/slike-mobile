...
class _OnScrollHideContentState extends State<OnScrollHideContent> {
  ...
  void _controllerListener() {
    ...
    if (widget.floating) {
      ...
      _buttonPosition.value = (_buttonHeight - pixels).clamp(0.0, _buttonHeight);
  ...
  void _updateHeight(double to) {
    ...
    _buttonPosition.value = Misc.lerpDouble(_buttonPositionRef, to, lerp <= 1.0 ? lerp : 1.0);
    widget.onChanged?.call(_buttonPosition.value);
  }
      ...
      child: SizeChangedLayoutNotifier(
        ...
        child: ValueListenableBuilder(
          ...
          builder: (_, double value, Widget? child) {
            final offset = widget.onTop ? value - _buttonHeight : _buttonHeight - value;
            final double opacity = (offset / _buttonHeight).clamp(0, 1);
            final Widget transform = Transform.translate(
              offset: Offset(0.0, offset),
              child: child,
            );
            return widget.opacity ? Opacity(opacity: opacity, child: transform) : transform;
          },
          child: widget.child,
        ),
      ),
...
class _BooleanTweenState<T> extends State<BooleanTween<T>> with SingleTickerProviderStateMixin {
...
class SwipeTransition extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return clip == Clip.none ? swipper : ClipRRect(clipBehavior: clip, child: swipper);
  }
}
    ...
    return BooleanTween<Offset>(
      ...
      builder: (_, offset, ___) {
        return FractionalTranslation(
          translation: textDirection == TextDirection.rtl ? Offset(-offset.dx, offset.dy) : offset,
