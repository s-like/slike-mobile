...
class VideoModel {
  ...
  static List<Video> parseVideos(jsonData) {
    print("parseAttributes $jsonData");
    DashboardService dashboardService = Get.find();
    List list = jsonData;
    List<Video> attrList = list.map((data) {
      dashboardService.postIds.add(data["video_id"] ?? 0);
...
class Video {
  ...
  Video.fromJSON(Map<String, dynamic> json) {
    print(json);
    print("json['sound_username'] ${json['sound_username']} ${json['thumb']}");
    ...
    aspectRatio = json["aspect_ratio"] != null ? num.parse((json["aspect_ratio"] ?? "0").toString()) : 0.0;
  }
